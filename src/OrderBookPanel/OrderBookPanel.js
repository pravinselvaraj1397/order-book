...
const OrderBookPanel = (props) => {
  ...
  const { base, quote, isPerp, uiID } = currentMarket;
  ...
  const snapshotReceived = useSelector((state) =>
    getBookSnapshotReceived(state, symbol)
  );
  ...
  const isSubscribedToSymbol = useSelector((state) =>
    isSubscribedToBook(state, symbol)
  );
